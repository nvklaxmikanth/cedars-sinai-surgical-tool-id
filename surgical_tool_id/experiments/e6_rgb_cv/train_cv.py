"""Train the E5 SmallCNN with true RGB inputs on E2 video folds.

Run one fold per process for isolated peak-memory measurements:
    python experiments/e6_rgb_cv/train_cv.py --fold 0
Then aggregate:
    python experiments/e6_rgb_cv/train_cv.py --aggregate
"""
...
class FrameDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        tensor = torch.tensor(list(image.getdata()), dtype=torch.float32).view(128, 128, 3)
...
def aggregate():
    ...
    e5_root = OUT.parent / "e5_weighted_ce_cv"
    e5_summary = json.loads((e5_root / "results.json").read_text())
    with (e5_root / "oof_predictions.csv").open(newline="") as file:
        e5_by_path = {row["path"]: row for row in csv.DictReader(file)}
    if set(e5_by_path) != {row["path"] for row in rows}:
        raise ValueError("E5 and E6 OOF paths do not match")
    changed = [row for row in rows if row["predicted_label"] != e5_by_path[row["path"]]["predicted_label"]]
    comparison = {
        "primary_metric": "pooled_macro_f1",
        "e5_pooled_macro_f1": e5_summary["pooled_metrics"]["macro_f1"],
        "e6_pooled_macro_f1": metrics["macro_f1"],
        "selected_experiment": "E6" if metrics["macro_f1"] > e5_summary["pooled_metrics"]["macro_f1"] else "E5",
        "e5_pooled_accuracy": e5_summary["pooled_metrics"]["accuracy"],
        "e6_pooled_accuracy": metrics["accuracy"],
        "changed_oof_predictions": len(changed),
        "wrong_to_correct": sum(e5_by_path[row["path"]]["predicted_label"] != row["true_label"]
                                and row["predicted_label"] == row["true_label"] for row in changed),
        "correct_to_wrong": sum(e5_by_path[row["path"]]["predicted_label"] == row["true_label"]
                                and row["predicted_label"] != row["true_label"] for row in changed),
    }
    (OUT / "comparison_to_e5.json").write_text(json.dumps(comparison, indent=2) + "\n")
