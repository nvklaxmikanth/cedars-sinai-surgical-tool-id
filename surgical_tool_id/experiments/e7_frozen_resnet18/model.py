"""Offline ResNet18 compatible with torchvision's IMAGENET1K_V1 state dict."""
...
WEIGHTS_PATH = Path(__file__).parent / "weights" / "resnet18-f37072fd.pth"
...
def load_frozen_backbone(num_classes=4):
    """Load bundled official weights without touching the network or cache."""
    actual = hashlib.sha256(WEIGHTS_PATH.read_bytes()).hexdigest()
    if actual != WEIGHTS_SHA256:
        raise ValueError(f"pretrained weight hash mismatch: {actual}")
    ...
    model.eval()  # freezes BatchNorm running statistics and batch counters
    return model


def backbone_state_hash(model):
    """Hash every backbone tensor, including BatchNorm buffers and counters."""
...
def predict_image(path, head_checkpoint, class_names):
    """Offline single-image inference with a selected fold head."""
    model = load_frozen_backbone(num_classes=len(class_names))
    model.fc.load_state_dict(torch.load(head_checkpoint, map_location="cpu", weights_only=True))
    model.eval()
    with torch.no_grad():
